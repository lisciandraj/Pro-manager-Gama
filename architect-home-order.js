...
window.addEventListener('gama:auth-change',e=>{if(e.detail?.event==='TOKEN_REFRESHED')return;epoch++;revision++;user=null;order=[];loading=null;queue=Promise.resolve();finish(true);if(grid){const defaults=window.GamaMenu?.items||[];const ranks=['dashboard','assistant-ia','crm','quotes','sales-orders','tms','payments','returns','gamaPurchasesV14','products','warehouses','projects','hr','knowledge','accounting','contacts','settings'];order=[...defaults].sort((a,b)=>(ranks.indexOf(a[1])<0?100:ranks.indexOf(a[1]))-(ranks.indexOf(b[1])<0?100:ranks.indexOf(b[1]))).map(x=>x[1]);apply()}if(e.detail?.event!=='SIGNED_OUT')load()});
