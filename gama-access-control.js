...
(function loadGamaCloud(){
  if(window.GamaCloud || window.__gamaCloudLoading){
    if(window.GamaCloud && !window.__gamaCentralSyncLoading){var cs=document.createElement('script');cs.src='gama-central-sync.js?v=20260913-payments1';cs.async=true;document.head.appendChild(cs);window.__gamaCentralSyncLoading=true;}
    return;
  }
  window.__gamaCloudLoading=true;
  var s=document.createElement('script');s.src='gama-supabase.js?v=20260913-refs1';s.async=true;
  s.onload=function(){window.dispatchEvent(new CustomEvent('gama:cloud-script-loaded'));var cs=document.createElement('script');cs.src='gama-central-sync.js?v=20260913-payments1';cs.async=true;document.head.appendChild(cs);window.__gamaCentralSyncLoading=true;};
...
const ROLES={
 ...
 commercial:{label:'Comercial',perms:['payments','dossier-flow','settings','operations','notifications','gamaPurchasesV14','dashboard','products','clients','billing','reports','suppliers','matrix','customer-requests','price-lists','hr','crm','warehouses','sales-orders','quotes','client-deliveries']},
 magasinier:{label:'Almacenero',perms:['dossier-flow','settings','operations','notifications','gamaPurchasesV14','dashboard','products','movement','stock','warehouses','barcode','tms','order-preparation','hr','sales-orders']},
