...
/* Categoría del cliente: A factura al precio grossiste de la ficha, B al
   precio al detalle y C al precio pactado en su tarifa. Un cliente C cae en
   el precio de la categoría A cuando el producto no está en su tarifa, igual
   que un proveedor C sin precio especial cae en su precio de compra A. */
...
async function selectClientForInvoiceCloud(){
 ...
 if(tag)tag.textContent=!c?'':contractCategory==='C'
  ?(c.priceListId?'Categoría C · tarifa del cliente aplicada':'Categoría C sin tarifa asignada: precio grossiste (A)')
  :(contractCategory==='B'?'Categoría B · precio al detalle':'Categoría A · precio grossiste');
}
