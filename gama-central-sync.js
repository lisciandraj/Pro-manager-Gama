...
const PRODUCT_COLUMNS='id,barcode,name,description,reference,category,family,lines,brand,presentation,location,supplier_id,min_stock,max_stock,qty_per_carton,weight_g,volume_cm3,stock,sale_price,price_a,price_b,tax_rate,active,has_photo,created_at';
function mapProduct(p){return {id:p.id,barcode:p.barcode||'',name:p.name||'',description:p.description||'',ref:p.reference||'',cat:p.category||'',family:p.family||'',lines:p.lines||'',brand:p.brand||'',presentation:p.presentation||'',loc:p.location||'',supplierId:p.supplier_id||'',min:Number(p.min_stock||0),maxStock:Number(p.max_stock||0),qtyCarton:Number(p.qty_per_carton||0),weightG:Number(p.weight_g||0),volumeCm3:Number(p.volume_cm3||0),stock:Number(p.stock||0),price:Number(p.sale_price||0),priceA:Number(p.price_a||0),priceB:Number(p.price_b||0),iva:Number(p.tax_rate??15),photo:p.photo_data||'',hasPhoto:p.photo_data?true:!!p.has_photo,active:p.active!==false}}
...
async function migrateLocalOnce(){
 ...
 for(const p of snap.products||[]){try{const r=await GamaCloud.insert('products',{barcode:p.barcode||null,name:p.name,description:p.description||null,reference:p.ref||null,category:p.cat||null,family:p.family||null,lines:p.lines||null,brand:p.brand||null,presentation:p.presentation||null,location:p.loc||null,min_stock:Number(p.min||0),max_stock:Number(p.maxStock||0),qty_per_carton:Number(p.qtyCarton||0),weight_g:Number(p.weightG||0),volume_cm3:Number(p.volumeCm3||0),stock:Number(p.stock||0),sale_price:Number(p.price||0),price_a:Number(p.priceA??p.purchase_price??0),price_b:Number(p.priceB||0),tax_rate:Number(p.iva??15),photo_data:p.photo||null,active:true});if(r.data)productMap[p.barcode]=r.data.id}catch(e){console.warn('Producto local no migrado',e)}}
