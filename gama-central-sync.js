...
function mapClient(c){return {paymentTermsDays:c.payment_terms_days??null,cloudId:c.id,category:c.category||'A',id:c.identification||'',name:c.name||'',idType:'RUC',address:c.address||'',phone:c.phone||'',email:c.email||'',city:c.city||'',province:c.province||'',notes:c.notes||'',active:c.active!==false}}
...
async function saveClientCloud(){const terms=$('cPaymentTerms').value.trim();if(!/^\d+$/.test(terms)||Number(terms)>3650)return alert('Indica un plazo de pago entre 0 y 3650 días.');const name=$('cName').value.trim(),id=$('cId').value.trim(),editing=$('editingClientId').value;if(!name||!id)return alert('El nombre y la identificación son obligatorios.');const old=editing?db.clients.find(c=>c.cloudId===editing||c.id===editing):null;if(!old&&db.clients.some(c=>c.id===id))return alert('Ya existe un cliente con esta identificación.');const row={payment_terms_days:Number(terms),name,identification:id,address:$('cAddress').value.trim()||null,phone:$('cPhone').value.trim()||null,email:$('cEmail').value.trim()||null,city:$('cCity').value.trim()||null,province:$('cProvince').value.trim()||null,category:$('cCategory')?.value||'A',notes:$('cNotes').value.trim()||null,active:true};const r=old?await GamaCloud.update('customers',old.cloudId,row):await GamaCloud.insert('customers',row);if(r.error){alert('No se pudo guardar el cliente en la base central: '+r.error.message);return}clearClientForm();await loadAll();window.dispatchEvent(new Event('gama:sales-change'));alert(old?'Cliente actualizado en la base central.':'Cliente guardado en la base central.')}
...
function populateClientSelectCloud(){const s=$('clientSelect');if(!s)return;const cur=s.value;s.innerHTML='<option value="" data-gi=57780de4ab62>Selecciona un cliente...</option>'+db.clients.map(c=>`<option value="${c.id}">${c.name} — ${c.id}</option>`).join('');if(cur&&db.clients.some(c=>c.id===cur))s.value=cur}
