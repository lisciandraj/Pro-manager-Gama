...
const PRODUCT_COLUMNS='id,barcode,name,reference,category,location,supplier_id,min_stock,stock,sale_price,purchase_price,tax_rate,active,has_photo,created_at';
function mapProduct(p){return {id:p.id,barcode:p.barcode||'',name:p.name||'',ref:p.reference||'',cat:p.category||'',loc:p.location||'',supplierId:p.supplier_id||'',min:Number(p.min_stock||0),stock:Number(p.stock||0),price:Number(p.sale_price||0),purchase_price:Number(p.purchase_price||0),iva:Number(p.tax_rate??15),photo:p.photo_data||'',hasPhoto:p.photo_data?true:!!p.has_photo,active:p.active!==false}}
...
async function createProductCloud(){const editing=$('editingBarcode')?.value,b=$('pBarcode')?.value.trim(),n=$('pName')?.value.trim();if(!b||!n)return alert('El código de barras y el nombre son obligatorios.');if(!editing&&db.products.some(p=>p.barcode===b))return alert('Este código de barras ya existe.');const old=editing?db.products.find(p=>p.barcode===editing):null;let photo=null;if($('pPhoto')?.files?.[0]&&window.compressPhoto)photo=await compressPhoto($('pPhoto').files[0]);const row={barcode:b,name:n,reference:$('pRef').value.trim()||null,category:$('pCat').value.trim()||null,location:$('pLoc').value.trim()||null,supplier_id:$('pSupplier')?.value||null,min_stock:Number($('pMin').value)||0,sale_price:Number($('pPrice').value)||0,purchase_price:Number($('pCost')?.value)||0,tax_rate:Number($('pIva').value),active:true};
