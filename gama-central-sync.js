...
async function loadAll(){if(!window.GamaCloud)return false;if(!isStaffSession())return false;loading=true;try{const [pr,cl,su,mv,iv,pf]=await Promise.all([cachedList('products',{select:PRODUCT_COLUMNS,order:'name',ascending:true}),cachedList('customers',{order:'name',ascending:true}),cachedList('suppliers',{order:'name',ascending:true}),cachedList('stock_movements',{order:'created_at',ascending:false}),cachedList('invoices',{order:'issue_date',ascending:false}),cachedList('profiles',{order:'created_at',ascending:true})]);if(pr.error||cl.error||su.error||mv.error||iv.error)throw(pr.error||cl.error||su.error||mv.error||iv.error);if(pf.error)console.warn('[GAMA CLOUD] profiles',pf.error);profilesById=Object.fromEntries((pf.data||[]).filter(x=>x.full_name).map(x=>[x.id,x.full_name]));const allP=(pr.data||[]).map(mapProduct),allC=(cl.data||[]).map(mapClient);db.products=allP.filter(x=>x.active!==false);db.archivedProducts=allP.filter(x=>x.active===false);db.clients=allC.filter(x=>x.active!==false);db.archivedClients=allC.filter(x=>x.active===false);db.suppliers=(su.data||[]).filter(x=>x.active!==false);window.ArcEntities.suppliersCache=db.suppliers;db.moves=(mv.data||[]).map(mapMove);const invIds=(iv.data||[]).map(i=>i.id);const linesRes=invIds.length?await window.ArcData.byIds('invoice_lines','invoice_id',invIds,{order:'id',ascending:true},true):{data:[]};if(linesRes.error)throw linesRes.error;const linesByInvoice=new Map();(linesRes.data||[]).forEach(l=>{const k=String(l.invoice_id);if(!linesByInvoice.has(k))linesByInvoice.set(k,[]);linesByInvoice.get(k).push(l)});db.invoices=(iv.data||[]).map(i=>mapInvoice(i,linesByInvoice.get(String(i.id))||[]));db.__cloud=true;ready=true;clearLocalBusinessData();try{renderAll()}catch(e){}return true}catch(e){console.error('[GAMA CLOUD]',e);return false}finally{loading=false}}
...
async function registerMovementCloud(){const p=db.products.find(x=>x.barcode===($('moveBarcode').value||'').trim()),q=Number($('moveQty').value),type=$('moveType').value;if(!p)return alert('Producto no encontrado.');if(q<1)return alert('Cantidad inválida.');const c=await GamaCloud.db();const {error}=await window.ArcData.rawRpc('gama_register_stock_movement',{p_product_id:p.id,p_type:type==='IN'?'in':type==='OUT'?'out':'adjustment',p_quantity:q,p_reason:$('moveReason').value,p_comment:$('moveComment').value||null});if(error){alert(error.message.includes('INSUFFICIENT_STOCK')?'Stock insuficiente.':('No se pudo registrar el movimiento: '+error.message));return}$('moveInfo').textContent=`${p.name} — movimiento registrado en Cloud.`;window.ArcData.invalidate();await loadAll();alert('Movimiento registrado y sincronizado para todos los usuarios.')}
async function createCorrectionCloud(){const b=prompt('Código de barras del producto a corregir:');if(!b)return;const p=db.products.find(x=>x.barcode===b);if(!p)return alert('Producto no encontrado.');const target=Number(prompt(`Stock actual: ${p.stock}. ¿Cuál debe ser el stock correcto?`));if(!Number.isFinite(target)||target<0)return alert('Valor inválido.');const delta=target-p.stock;if(delta===0)return alert('No hay diferencia de stock.');const comment=prompt('Motivo obligatorio de la corrección:')||'';if(!comment)return alert('La corrección requiere un comentario.');const c=await GamaCloud.db();const {error}=await window.ArcData.rawRpc('gama_register_stock_movement',{p_product_id:p.id,p_type:'adjustment',p_quantity:delta,p_reason:'Corrección de inventario',p_comment:comment});if(error){alert('No se pudo registrar la corrección: '+error.message);return}window.ArcData.invalidate();await loadAll();alert('Corrección registrada centralmente y visible para todos los usuarios.')}
