...
function inject(){if($('gamaClientCatalogStyle'))return;const s=document.createElement('style');s.id='gamaClientCatalogStyle';s.textContent=`#client-catalog{display:none}#client-catalog.active{display:block}.cc{container-type:inline-size;background:#fff;border:1px solid var(--arc-line);border-radius:16px;padding:16px;box-shadow:0 3px 16px rgba(18,37,60,.05)}.ccTools{display:flex;gap:9px;align-items:center;margin:16px 0;flex-wrap:wrap}.ccTools input{flex:1 1 260px;min-width:0;width:auto}.ccTools select{flex:0 0 auto;width:240px;min-width:0;padding:11px;border-radius:10px;border:1px solid var(--arc-line)}.ccTools .badge{flex:0 0 auto;white-space:nowrap}
...
.ccGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.ccGrid>.gamaPager{grid-column:1/-1}
.ccProduct{display:grid;min-width:0;grid-template-columns:minmax(0,1fr);grid-template-rows:auto 1fr auto auto;grid-template-areas:'media' 'info' 'precio' 'acciones';gap:10px;border:1px solid var(--arc-line);border-radius:14px;padding:14px;background:#fff;box-shadow:0 1px 2px rgba(18,37,60,.04);transition:box-shadow .12s ease,border-color .12s ease}
.ccProduct:hover{border-color:var(--arc-line-strong);box-shadow:0 2px 4px rgba(18,37,60,.06),0 10px 24px rgba(18,37,60,.07)}
.ccMedia{grid-area:media;position:relative;display:grid;place-items:center}
.ccProduct img,.ccNoImg{width:100%;height:140px;object-fit:contain;border-radius:10px;background:var(--arc-surface-2)}
.ccNoImg{display:grid;place-items:center;font-size:42px}
.ccFlag{position:absolute;top:6px;left:6px;background:var(--arc-accent-100);color:var(--arc-accent-600);border-radius:999px;padding:3px 9px;font-size:10px;font-weight:850}
.ccInfo{grid-area:info;min-width:0}
.ccProduct h3{overflow-wrap:anywhere;font-size:15px;margin:0 0 3px;line-height:1.25;color:var(--arc-text)}
.ccBarcode{font-size:12px;color:var(--arc-text-muted);overflow-wrap:anywhere;margin-top:5px}.ccMeta{display:flex;flex-wrap:wrap;gap:4px 10px;font-size:11.5px;color:var(--arc-text-muted);margin-top:6px;overflow-wrap:anywhere}
.ccStock{display:inline-block;margin-top:10px;font-size:11px;font-weight:800;border-radius:999px;padding:4px 10px;background:var(--arc-success-bg);color:var(--arc-success)}
.ccStock.bajo{background:var(--arc-warning-bg);color:var(--arc-warning)}
.ccStock.agotado{background:var(--arc-surface-2);color:var(--arc-text-muted)}
.ccPrecio{grid-area:precio;text-align:left;align-self:end}
.ccPrecio b{display:block;font-size:23px;font-weight:900;color:var(--arc-text);letter-spacing:-.4px;line-height:1.15}
.ccPrecio b span{font-size:12.5px;font-weight:750;color:var(--arc-text-muted);letter-spacing:0}
.ccPrecioIva{font-size:13px;color:var(--arc-text-muted);margin-top:2px}
.ccPrecio small{display:block;font-size:11px;color:var(--arc-text-subtle);margin-top:5px}
.ccAcciones{grid-area:acciones;display:flex;gap:10px;align-items:center;margin-top:0;padding-top:10px;border-top:1px solid var(--arc-surface-3)}
.ccAdd{flex:1;background:var(--arc-warning);color:#fff;font-weight:800;font-size:12px;min-height:44px;min-width:0;border-radius:10px;padding:8px;display:flex;align-items:center;justify-content:center;gap:8px}
.ccAdd:hover:not(:disabled){background:var(--arc-warning)}
.ccAdd.dentro{background:var(--arc-success)}
.ccAdd.dentro:hover:not(:disabled){background:var(--arc-success)}
.ccQty{display:grid;grid-template-columns:32px 42px 32px;gap:0;flex:0 0 auto;border:1px solid var(--arc-line);border-radius:10px;overflow:hidden}
.ccQty button{background:#fff;color:var(--arc-text);border-radius:0;font-size:17px;padding:11px 0}
.ccQty button:hover{background:var(--arc-surface-3)}
.ccQty input{min-width:0;width:100%;text-align:center;border:0;border-left:1px solid var(--arc-line);border-right:1px solid var(--arc-line);border-radius:0;font-size:15px;font-weight:700;padding:11px 0}
.ccQty input:focus{box-shadow:none;outline:2px solid var(--arc-accent-600);outline-offset:-2px}.ccCartQty{grid-template-columns:32px 1fr 32px;gap:4px}.ccCartQty button{padding:6px 0;font-size:14px}.ccCartQty input{padding:6px 2px;font-size:13px}.ccCartRow small{display:block;color:var(--arc-text-subtle);font-size:11px;margin-top:2px}.ccCart{margin-top:18px;border:1px solid var(--arc-line);border-radius:14px;background:var(--arc-surface-2);padding:14px}.ccCartHead{display:flex;justify-content:space-between;align-items:center}.ccCartHead h3{margin:0}.ccCartRow{display:grid;grid-template-columns:1fr 122px 85px 32px;gap:8px;align-items:center;padding:8px 0;border-bottom:1px solid var(--arc-line);font-size:12px}.ccCartRow:last-child{border-bottom:0}
.ccResumen{margin:12px 0 4px;padding-top:11px;border-top:1px solid var(--arc-line);font-size:13px}
.ccResumen div{display:flex;justify-content:space-between;padding:4px 0;color:var(--arc-text-muted)}
.ccResumen b{color:var(--arc-text)}
.ccResumenTotal{margin-top:4px;padding-top:8px!important;border-top:1px solid var(--arc-line);font-size:15px;font-weight:800;color:var(--arc-text)}
.ccResumenTotal b{font-size:17px;color:var(--arc-accent-600)}.ccDel{background:var(--arc-danger-bg);color:var(--arc-danger);padding:6px}.ccMsg{margin-top:8px;font-size:12px;font-weight:700}.ccOk{color:var(--arc-success)}.ccErr{color:var(--arc-danger)}.ccFav{margin-top:14px;border:1px solid var(--arc-line);border-radius:14px;background:#fff;padding:14px}.ccFavHead{display:flex;justify-content:space-between;align-items:center}.ccFavHead h3{margin:0;font-size:15px}.ccFavRow{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--arc-surface-3);font-size:12px}.ccFavRow:last-child{border-bottom:0}.ccFavActions{display:flex;gap:6px}@container(max-width:900px){.ccGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@container(max-width:580px){.ccGrid{grid-template-columns:minmax(0,1fr)}.ccProduct img,.ccNoImg{height:150px}}
...
function ensureUI(){inject();let sec=$('client-catalog');if(!sec){sec=document.createElement('section');sec.id='client-catalog';(document.querySelector('.wrap')||document.body).appendChild(sec)}if(sec.dataset.ready)return;sec.dataset.ready='1';sec.innerHTML=`${window.GamaUI.header({title:'🛍️ Catálogo de productos',lead:'Elija sus productos y envíe el pedido a Architect.'})}<div class="cc"><div class="ccTools"><input id="ccSearch" data-gi-placeholder=48d9573a38c4 placeholder="Buscar un producto…"><select id="ccCategory"><option value="" data-gi=425a839def0b>Todas las categorías</option></select><span class="badge" id="ccCount">0 productos</span></div><div class="muted" id="ccStatus"><span class="gamaSpin"></span>Cargando…</div><div class="ccGrid" id="ccProducts"></div><div class="ccFav" id="ccFav"><div class="ccFavHead"><h3 data-gi=cb6b4fa7ebba>⭐ Mis pedidos favoritos</h3></div><div id="ccFavRows" class="muted" data-gi=73a81c321e99>Sin favoritos guardados.</div></div><div class="ccCart"><div class="ccCartHead"><h3 data-gi=af887267f60d>🛒 Mi pedido</h3><b id="ccTotal">$0,00</b></div><div id="ccCartRows" style="margin:8px 0"><span class="muted" data-gi=a63b1e7d1713>Su pedido está vacío.</span></div><div class="ccResumen"><div><span data-gi=53de02aa48c6>Subtotal sin IVA</span><b id="ccNeto">$0,00</b></div><div><span data-gi=ba6da46c5e1c>IVA</span><b id="ccIva">$0,00</b></div><div class="ccResumenTotal"><span data-gi=0db5bb8bb2fe>Total con IVA</span><b id="ccBruto">$0,00</b></div></div><textarea id="ccNotes" data-gi-placeholder=0ada0d45e88a placeholder="Comentario o indicación para el pedido…"></textarea><button class="primary" id="ccSend" style="width:100%;margin-top:9px" data-gi=10b4a1190405>Enviar pedido</button><button class="secondary" id="ccSaveFav" style="width:100%;margin-top:8px" data-gi=580cb2abec1a>☆ Guardar como favorito</button><div id="ccMsg" class="ccMsg"></div></div></div>`;window.GamaUI.bindBack(sec);$('ccSearch').oninput=()=>{GamaPage.reset('catalog');render()};$('ccCategory').onchange=()=>{GamaPage.reset('catalog');render()};GamaPage.register('catalog',render);$('ccSend').onclick=send;$('ccSaveFav').onclick=saveFavorite}
function populateCategorySelect(){const s=$('ccCategory');if(!s)return;const cur=s.value;const cats=[...new Set(products.map(p=>(p.category||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'));s.innerHTML='<option value="" data-gi=425a839def0b>Todas las categorías</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');if(cur&&cats.includes(cur))s.value=cur}
...
<div class="ccQty"><button type="button" data-minus="${esc(p.id)}" data-gi-aria-label=ffa160172bac aria-label="Quitar uno">−</button><input type="number" min="0" value="${qty}" data-input="${esc(p.id)}" aria-label="Cantidad de ${esc(p.name)}"><button type="button" data-plus="${esc(p.id)}" data-gi-aria-label=05193a4d8db8 aria-label="Añadir uno">+</button></div>
<button type="button" class="ccAdd${qty?' dentro':''}" data-add="${esc(p.id)}" data-gi-live>${qty?'✓ En el pedido ('+qty+')':'🛒 Añadir al pedido'}</button>
</div></article>`;
}
function render(){const q=($('ccSearch')?.value||'').toLowerCase().trim(),cat=$('ccCategory')?.value||'',rows=products.filter(p=>(!q||[p.name,p.reference,p.barcode,p.category].some(v=>String(v||'').toLowerCase().includes(q)))&&(!cat||p.category===cat));$('ccCount').textContent=rows.length+' producto'+(rows.length>1?'s':'');$('ccProducts').innerHTML=rows.length?GamaPage.slice('catalog',rows).map(ficha).join('')+GamaPage.controls('catalog',rows.length):'<div class="muted" data-gi=31f22142a6d8>No se encontraron productos.</div>';$('ccProducts').querySelectorAll('[data-minus]').forEach(b=>b.onclick=()=>qty(b.dataset.minus,(cart.find(x=>x.product_id===b.dataset.minus)?.quantity||0)-1));$('ccProducts').querySelectorAll('[data-plus]').forEach(b=>b.onclick=()=>qty(b.dataset.plus,(cart.find(x=>x.product_id===b.dataset.plus)?.quantity||0)+1));$('ccProducts').querySelectorAll('[data-input]').forEach(i=>i.onchange=()=>qty(i.dataset.input,Number(i.value)||0));
...
function renderCart(){
 ...
 host.innerHTML=cart.length?cart.map(x=>`<div class="ccCartRow"><div><b>${esc(x.name)}</b><small>${money(x.unit_price)} c/u</small></div><div class="ccQty ccCartQty"><button class="secondary" data-cminus="${esc(x.product_id)}" data-gi-aria-label=ffa160172bac aria-label="Quitar uno">−</button><input type="number" min="0" step="1" value="${x.quantity}" data-cinput="${esc(x.product_id)}" aria-label="Cantidad de ${esc(x.name)}"><button class="primary" data-cplus="${esc(x.product_id)}" data-gi-aria-label=05193a4d8db8 aria-label="Añadir uno">+</button></div><span>${money(x.quantity*x.unit_price)}</span><button class="ccDel" data-del="${esc(x.product_id)}" data-gi-aria-label=aa7484780ab1 aria-label="Quitar del pedido">×</button></div>`).join(''):'<span class="muted" data-gi=a63b1e7d1713>Su pedido está vacío.</span>';
...
function show(){if(!window.gamaAccessAllowed?.('client-catalog'))return;ensureUI();document.querySelectorAll('section').forEach(s=>{const active=s.id==='client-catalog';s.classList.toggle('active',active);s.hidden=!active;s.style.display=active?'block':'none'});load()}
