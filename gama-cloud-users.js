...
function patch(){
  ...
  s.innerHTML=`${window.GamaUI.header({kicker:'GAMA Cloud',title:'👥 Usuarios y accesos',lead:'Quién puede entrar en GAMA y con qué permisos. Los usuarios que crees desde «Cuentas cloud» aparecen aquí solos, y cualquier cambio de rol lo ven al instante los demás administradores conectados.',actions:'<button type="button" class="gamaStdAction" id="cuRefresh">↻ Actualizar</button>'})}<div class="card cloudUsersV17"><div class="cuBar"><h3>Usuarios cloud</h3><span class="cuOnline"><i></i> Cloud conectado</span></div><div id="cuPending" class="cuPending" hidden></div><div id="cuStatus" class="cuStatus">Cargando usuarios…</div><div class="cuTable"><table><thead><tr><th>Nombre</th><th>Email</th><th>Rol</th><th>Estado</th><th>Creado</th><th>Acciones</th></tr></thead><tbody id="cuRows"></tbody></table></div><div class="cuFoot"><span id="cuCount">0 usuarios</span><span>● Actualización automática activa</span></div></div>`;
  window.GamaUI.bindBack(s);document.getElementById('cuRefresh').onclick=()=>load();
