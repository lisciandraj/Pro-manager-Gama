...
function renderDetail(d,x){
 ...
 const financeAction=d.o?(p.done&&d.q&&!f.rows.length?btn('Crear factura interna','invoice',d.q.id,'billing'):'')+btn('Ver facturas y registrar cobros','finance',d.o.id,'billing'):'';
 if(financial)steps.push(
 step('9. Facturación interna',f.invoiced?'done':f.rows.length?'active':'pending',['Validar la entrega completa y su prueba firmada antes de crear la factura interna.','Cubrir todas las líneas del pedido. El número oficial externo es opcional.'],`${tr('Facturado')} : ${money(f.billed/100)} · ${tr('Pendiente de facturar')} : ${money(f.unbilled/100)}`,financeAction),
