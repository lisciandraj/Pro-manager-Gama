...
function financialProgress(x,today){
 ...
 const invoiced=covered&&!externalPending;
...
function renderDetail(d,x){
 ...
 step('9. Facturación interna y externa',f.invoiced?'done':f.rows.length?'active':'pending',['Crear la factura interna o vincular la factura emitida en el software externo.','Cubrir todas las líneas del pedido y verificar la referencia externa.','La referencia externa de una factura interna no genera un segundo importe.'],`${tr('Facturado')} : ${money(f.billed/100)} · ${tr('Pendiente de facturar')} : ${money(f.unbilled/100)}${f.externalPending?'<br>'+tr('Referencia externa pendiente de registrar o verificar.'):''}`,financeAction),
