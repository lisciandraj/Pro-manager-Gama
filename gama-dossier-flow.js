...
function shell(){
 if(!$('gdfStyle')){const s=document.createElement('style');s.id='gdfStyle';document.head.appendChild(s)}
 let s=$(ID);if(!s){s=document.createElement('section');s.id=ID;(document.querySelector('.wrap')||document.body).appendChild(s)}
 window.ArcUI.render(s,GamaUI.header({title:'Seguimiento de expedientes',lead:'De la solicitud del cliente al cobro: avance, necesidades y requisitos.'})+`<div class="gdfTools"><input id="gdfSearch" data-gi-aria-label=320b429c7b40 aria-label="Buscar expediente" data-gi-placeholder=320b429c7b40 placeholder="Buscar expediente" data-gi-live><button class="arcButton secondary" id="gdfRefresh">${tr('Actualizar')}</button></div><div class="gdfLayout"><div id="gdfList" class="gdfList"></div><div id="gdfDetail" aria-live="polite"></div></div>`);
...
function renderList(){
 ...
 window.ArcUI.render($('gdfList'),a.map(d=>`<button class="arcButton gdfRecord" data-record="${esc(d.key)}" aria-pressed="${selected===d.key}"><b>${esc(ref(d))}</b><small>${esc(customer(d))}</small><small>${tr(d.o?'Pedido de venta':d.q?'Presupuesto':'Solicitud de cliente')}</small></button>`).join('')||tr('No hay expedientes disponibles.'));
 $('gdfList').querySelectorAll('[data-record]').forEach(b=>b.onclick=()=>detail(b.dataset.record));
}
async function open(key=null){
 if(!can(ID))return;selected=key;shell();const token=++generation;window.ArcUI.render($('gdfDetail'),tr('Cargando…'));
 ...
 if(token!==generation||!can(ID))return;records=group(orders,quotes,requests);renderList();if(records.length)await detail(records.find(d=>d.key===key)?.key||records[0].key);else window.ArcUI.render($('gdfDetail'),tr('Los expedientes aparecerán al registrar una solicitud, un presupuesto o un pedido.'));
 }catch(e){if(token===generation)failure()}
}
function failure(){ window.ArcUI.render($('gdfDetail'),`<p role="alert" class="gdfError">${tr('No se pudo cargar el expediente. Actualiza para reintentar; el avance no está confirmado.')}</p>`); }
async function detail(key){
 if(!can(ID))return;const d=records.find(x=>x.key===key);if(!d)return;selected=key;renderList();const token=++generation;window.ArcUI.render($('gdfDetail'),tr('Cargando…'));
...
function renderDetail(d,x){
 ...
 step('3. Validación y pedido',pending?'done':d.q?.quote_state==='sent'?'active':'pending',['Registrar la aceptación del cliente o confirmar el pedido directo.','Para una aceptación externa, indicar el medio y la referencia del acuerdo.'],d.q?.quote_state==='sent'&&d.q.quote_valid_until&&d.q.quote_valid_until<new Intl.DateTimeFormat('en-CA',{timeZone:(globalThis.window?.GamaCompany?.get()?.timezone||'America/Guayaquil'),year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date())?tr('Presupuesto vencido: revisar la vigencia antes de aceptar.'):esc(d.o?.number||''),orderAction|| (d.q?btn('Registrar validación','quote',d.q.id,'quotes'):'')),
 step('4. Stock y reservas',!pending?'pending':p.missing>0?'blocked':p.ordered?'done':'blocked',['Reservar existencias para las cantidades pendientes.','Reponer faltantes o registrar un acuerdo de entrega parcial con el cliente.'],`${tr('Cantidad sin reservar')} : ${p.missing}<p>${(x.reservations||[]).map(r=>esc(r.erp_reference||r.dossier_reference||'')).filter(Boolean).join(' · ')}</p>`,orderAction),
 step('5. Preparación',p.complete?'done':p.active.length?'active':'pending',['Registrar las cantidades preparadas del lote reservado.','Escanear los productos de cada cartón y cerrar la preparación antes de enviar al TMS.','Una expedición parcial necesita el motivo y respetar el acuerdo del cliente.'],`${tr('Lote activo: previsto / preparado')} : ${p.planned} / ${p.picked}`,prepAction),
 step('6. Expedición y salida',p.complete&&x.ships.every(s=>s.departed_at)?'done':x.ships.length?'active':'pending',['Recibir en TMS los bultos preparados y cerrados.','Asignar conductor y vehículo y confirmar la salida de los bultos, sin volver a escanear los productos.'],`${tr('Expedido / pedido')} : ${p.shipped} / ${p.ordered}`,orderAction),
 ...
 /* Flujo inverso: no es un paso más del expediente, es una rama que sólo
    aparece cuando hay una devolución. */
 const rets=x.returns||[];
 if(rets.length)steps.push(step('11. Devolución (flujo inverso)',
  rets.every(r=>['closed','cancelled'].includes(r.status))?'done':'active',
  ['Registrar la recepción física de la mercancía devuelta.',
   'Decidir producto por producto: stock, rebut o devolución al proveedor.',
   'Elegir la acción financiera —ninguna, abono, reembolso o crédito— y cerrar.'],
  rets.map(r=>esc(r.number)+' · '+tr(({to_process:'Por tratar',received:'Recibida',processed:'Tratada',closed:'Cerrada',cancelled:'Anulada'})[r.status]||r.status)).join('<br>'),
  btn('Abrir las devoluciones','returns',rets[0].id,'returns')));
 if(financial)steps.push(
 step('9. Facturación interna',f.invoiced?'done':f.rows.length?'active':'pending',['La factura interna se genera automáticamente al validar la última entrega con la firma del cliente.','Cubrir todas las líneas del pedido. El número oficial externo es opcional.'],`${tr('Facturado')} : ${money(f.billed/100)} · ${tr('Pendiente de facturar')} : ${money(f.unbilled/100)}`,financeAction),
 step('10. Pago y cierre del expediente',f.overdue?'blocked':f.settled&&p.done?'done':f.rows.length?'active':'pending',['Registrar cada pago con fecha, importe, medio y referencia.','Solo los cobros confirmados reducen el saldo; los pagos anulados se excluyen.','Saldar cada factura y completar la facturación antes de cerrar el expediente.'],`${tr('Cobrado')} : ${money(f.paid/100)} · ${tr('Saldo pendiente')} : ${money(f.balance/100)} · ${tr('Importe vencido')} : ${money(f.overdue/100)}`,d.o?btn('Abrir pagos del expediente','payments',d.o.id,'payments'):'')
 );
 const financePanel=financial?`<div class="arcPanel card" id="gdfFinance"><h3>${tr('Facturas y pagos del expediente')}</h3><p>${tr('La factura interna se genera automáticamente al validar la última entrega con la firma del cliente.')}</p>${f.rows.map(i=>`<div class="gdfDelivery"><b>${esc(i.number)}</b> · ${tr(i.document_kind==='internal'?'Factura interna':'Factura externa')}<p>${tr('Referencia externa')} : ${esc(i.external_number||(i.document_kind==='internal'?'—':i.number))}<br>${tr('Vencimiento')} : ${esc(i.due_date||'—')} ${i.overdue?tr('Vencida'):''}</p><p>${tr('Total')} : ${money(i.total)} · ${tr('Cobrado')} : ${money(i.paid/100)} · ${tr('Saldo pendiente')} : ${money(i.balance/100)}</p>${btn('Ver factura y cobros','payment',i.id,'payments')}</div>`).join('')||tr('Aún no hay facturas vinculadas.')}<div class="gdfTools">${financeAction}</div></div>`:'';
 window.ArcUI.render($('gdfDetail'),`<div class="arcPanel card"><h2>${esc(ref(d))}</h2><p>${esc(customer(d))}</p><p>${esc(d.o?.delivery_address||'')}</p><b>${tr(closed?'Expediente cerrado':p.done?(financial?(f.settled?'Expediente cerrado · entregado, facturado y pagado':'Entrega completa; seguimiento financiero pendiente'):'Entrega completa con prueba'):'Expediente en curso')}</b><p>${tr('El avance se calcula desde los documentos vinculados. Los controles se validan en cada módulo al ejecutar la acción.')}</p></div><div class="arcPanel card"><b>${tr('Próxima acción')}</b><p>${tr(next)}</p>${d.o?`<details><summary>${tr('Cantidades por producto')}</summary>${x.lines.map(l=>{const shipped=sum(x.shipLines.filter(s=>s.order_line_id===l.id));const reserved=sum(x.reservations.filter(r=>r.status==='active'&&x.links.some(k=>k.line_id===l.id&&k.reservation_id===r.id)));return `<p><b>${esc(l.product_name)}</b><br>${tr('Pedido / expedido / reservado / faltante')} : ${n(l.quantity)} / ${shipped} / ${reserved} / ${Math.max(0,n(l.quantity)-shipped-reserved)}</p>`}).join('')}</details>`:''}</div><ol class="gdfFlow">${steps.join('')}</ol>${financePanel}<div class="arcPanel card"><h3>${tr('Expediciones y entregas')}</h3>${x.ships.map(s=>{const t=x.transport.find(t=>t.id===s.tms_delivery_id);return `<div class="gdfDelivery"><b>${esc(s.number)}</b><p>${tr(t?.status||'Estado no disponible')} · ${esc(t?.delivery_date||'—')} ${t&&late(t)?tr('Entrega atrasada'):''}</p><div class="gdfTools">${btn('Ver en TMS','delivery',s.tms_delivery_id,'tms')}${btn('Ver prueba de entrega','proof',s.tms_delivery_id,'sales-orders')}</div></div>`}).join('')||tr('Aún no hay expediciones.')}</div>`);
 window.ArchitectProjectControls?.mountDossier(d.key,$('gdfDetail'));
 $('gdfDetail').querySelectorAll('[data-action]').forEach(b=>b.onclick=async()=>{if(!can(ID))return;try{const id=b.dataset.id;switch(b.dataset.action){case'preparation':if(can('order-preparation'))await GamaPreparation.open(id);break;case'returns':if(can('returns'))await window.GamaReturns?.openReturn(id);break;case'invoice':if(financeAllowed())await GamaInternalInvoices.create(id,()=>detail(selected));break;case'finance':if(financeAllowed()){await GamaSales.openOrder(id);document.getElementById('gsLinkedInvoices')?.scrollIntoView({block:'start'});}break;case'payment':if(can('payments'))await GamaPayments.open({invoiceId:id});break;case'payments':if(can('payments'))await GamaPayments.open({orderId:id});break;case'order':if(can('sales-orders'))await GamaSales.openOrder(id);break;case'quote':if(can('quotes')){await GamaQuotes.open();await GamaQuotes.view(id)}break;case'request':if(can('customer-requests'))await window.GamaOpenCustomerRequest(id);break;case'delivery':if(can('tms'))await gamaTMS.openDelivery(id);break;case'proof':if(can('sales-orders'))await GamaFulfillment.proof(id);break}}catch(e){window.gamaToast?.(GamaI18n.t('No se pudo abrir el documento.'))}});
}
window.GamaDossierFlow={open,group,progress,financialProgress};
window.addEventListener('gama:sales-change',()=>{if(can(ID)&&$(ID)?.classList.contains('active'))open(selected)});
