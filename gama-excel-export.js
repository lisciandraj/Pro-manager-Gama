...
const groups=[
 ...
 ['Flotte','Flota','Fleet',[]],
 ['SAV','Servicio posventa','After-sales',['service_tickets','service_messages','service_events']],
...
const exportModules=['products','contacts','contacts','price-lists','quotes','client-catalog','crm','sales-orders','tms','sales-orders','payments','gamaPurchasesV14','warehouses','warehouses','tms','returns','hr','knowledge','audit','settings','billing','projects','accounting','fleet','sav','documents'];
...
async function run(){if(busy)return;busy=true;const buttons=[...document.querySelectorAll('[onclick="exportExcel()"]')];buttons.forEach(b=>b.disabled=true);
 ...
 await loadXLSX();const client=await GamaCloud.db(),sheets=groups.map(()=>[]),summary=[[tr('Export Architect ERP','Exportación Architect ERP','Architect ERP export'),new Date().toISOString()],[tr('Périmètre','Alcance','Scope'),tr('Données accessibles au compte connecté. Photos et fichiers binaires exclus. Textes longs répartis en colonnes numérotées.','Datos accesibles a la cuenta conectada. Sin fotos ni archivos binarios. Textos largos en columnas numeradas.','Data accessible to the signed-in account. No photos or binary files. Long text continues in numbered columns.')],[],['Module','Table','Rows','Status']];let denied=0;const exportRights=new Map();
 for(const [i,spec] of window.GamaExportSchema.entries()){const n=group(spec.table),name=groups[n][{fr:0,es:1,en:2}[lang()]??1];status(tr('Lecture','Lectura','Reading')+' '+(i+1)+'/'+GamaExportSchema.length+' — '+name);try{if(window.ArchitectAccessControls){const module=spec.table.startsWith('external_invoice')?'payments':spec.table==='commercial_matrix'?'matrix':exportModules[n];if(!exportRights.has(module))exportRights.set(module,await window.ArcData.rpc('gama_action_allowed',{p_module:module,p_action:'export'}));if(!exportRights.get(module))throw Object.assign(Error('EXPORT_NOT_ALLOWED'),{code:'42501'})}const rows=await readTable(client,spec);for(const row of block(spec,rows))sheets[n].push(row);summary.push([name,spec.table,rows.length,'OK']);}catch(e){if(e.code==='42501'){denied++;summary.push([name,spec.table,'',tr('Accès refusé','Acceso denegado','Access denied')]);}else throw e;}}
 ...
 XLSX.writeFile(wb,'Architect_export_'+new Date().toISOString().replace(/[:.]/g,'-')+'.xlsx',{compression:true});status(denied?tr('Export terminé. Consultez le sommaire pour les accès refusés.','Exportación terminada. Consulte los accesos denegados en el resumen.','Export complete. See summary for denied access.'):tr('Export Excel terminé.','Exportación Excel terminada.','Excel export complete.'));
