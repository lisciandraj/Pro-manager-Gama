...
function css(){if(document.getElementById('gamaExcelCss'))return;const s=document.createElement('style');s.id='gamaExcelCss';s.textContent=`#excel-import-module,.gamaExcelModule{max-width:1100px;margin:auto}.gamaExcelBody{background:#fff;border:1px solid var(--arc-line);border-radius:18px;padding:18px}.gamaExcelDrop{border:2px dashed var(--arc-line-strong);border-radius:16px;padding:30px;text-align:center;background:var(--arc-surface-2);cursor:pointer}.gamaExcelDrop input{display:none}.gamaExcelIcon{font-size:38px;color:var(--arc-accent-600)}.gamaExcelDrop b,.gamaExcelDrop span{display:block}.gamaExcelDrop span{font-size:13px;color:var(--arc-text-muted);margin-top:5px}.gamaExcelTypes{display:flex;gap:7px;flex-wrap:wrap;margin:10px 0}.gamaExcelTypes button{background:var(--arc-surface-3);color:var(--arc-text)}.gamaExcelTypes button.active{background:var(--arc-accent-600);color:#fff}.gamaExcelInfo{margin-top:10px;color:var(--arc-text-muted);font-size:13px}.gamaExcelPreview{overflow:auto;max-height:320px;border:1px solid var(--arc-surface-3);border-radius:10px;margin-top:12px}.gamaExcelPreview table{width:100%;border-collapse:collapse}.gamaExcelPreview th,.gamaExcelPreview td{padding:8px;border-bottom:1px solid var(--arc-surface-3);text-align:left;white-space:nowrap;font-size:12px}.gamaExcelActions{display:flex;gap:8px;justify-content:flex-end;margin-top:12px}.gamaExcelActions .primary{background:var(--arc-accent-600);color:#fff}.gamaExcelActions .secondary{background:var(--arc-surface-3);color:var(--arc-text)}.gamaExcelModes{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:12px}.gamaExcelModes button{background:#fff;border:1px solid var(--arc-line-strong);color:var(--arc-text);border-radius:999px;padding:10px 16px;font-weight:800;cursor:pointer}.gamaExcelModes button.active{background:var(--arc-accent-600);border-color:var(--arc-accent-600);color:#fff}.gamaPhotoHint{background:var(--arc-surface-2);border:1px solid var(--arc-line);border-radius:12px;padding:12px;font-size:13px;color:var(--arc-text-muted);margin-top:12px}.gamaPhotoHint code{background:var(--arc-surface-3);border-radius:5px;padding:1px 5px;font-size:12px}.gamaPhotoOpt{display:flex;align-items:center;gap:8px;margin-top:12px;font-size:13px;color:var(--arc-text-muted)}.gamaPhotoOpt input{width:auto;margin:0}.gamaPhotoThumb{width:42px;height:42px;object-fit:cover;border-radius:7px;border:1px solid var(--arc-line);display:block}.gamaPhotoOk{color:var(--arc-success);font-weight:800}.gamaPhotoBad{color:var(--arc-danger);font-weight:800}.gamaPhotoWarn{color:var(--arc-warning);font-weight:800}@media(max-width:700px){.gamaExcelActions{display:grid;grid-template-columns:1fr}.gamaExcelDrop{padding:24px 12px}}`;document.head.appendChild(s);}
function render(){const h=host();if(!h)return;h.classList.add('gamaExcelModule');h.innerHTML=`${window.GamaUI.header({title:'📥 Importar datos',lead:'Importa datos desde Excel y fotos de tus productos.'})}<div class="gamaExcelModes"><button type="button" data-mode="data" class="active" data-gi=bf88a9b80c77>📄 Datos desde Excel</button><button type="button" data-mode="photos" data-gi=10ba210e1cd9>🖼️ Fotos de productos</button><button type="button" data-mode="optimize" data-gi=ae6f37333113>🗜️ Optimizar fotos</button></div><div class="gamaExcelBody" id="gamaExcelPanelData"><div class="gamaExcelDrop" id="gamaExcelDrop"><div class="gamaExcelIcon">▦</div><b data-gi=37b772c6e8fa>Selecciona o arrastra un archivo Excel</b><span data-gi=18ed7e649564>Formatos compatibles: .xlsx, .xls y .csv</span><input id="gamaExcelFile" type="file" accept=".xlsx,.xls,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv"></div><div id="gamaExcelStatus" class="gamaExcelInfo" data-gi=5b5c815f3c1f>Ningún archivo seleccionado.</div><div class="gamaExcelTypes"><button type="button" data-type="products" class="active" data-gi=90b19312a85b>📦 Productos</button><button type="button" data-type="clients" data-gi=4a756c24fe34>👥 Clientes</button><button type="button" data-type="suppliers" data-gi=5e4d8f37fe5e>🚚 Proveedores</button><button type="button" data-type="customerPrices" data-gi=e71e61b9dadc>🏷️ Tarifas de cliente</button></div><div id="gamaExcelPreview" class="gamaExcelPreview" data-gi=8810c4c2df6b>Selecciona un archivo para mostrar una vista previa.</div><div class="gamaExcelActions"><button type="button" id="gamaExcelTemplate" class="secondary" data-gi=d02205871bd5>Descargar plantilla</button><button type="button" id="gamaExcelImport" class="primary" disabled data-gi=63e31998d5d9>Importar datos</button></div></div><div class="gamaExcelBody" id="gamaExcelPanelPhotos" hidden><div class="gamaExcelDrop" id="gamaPhotoDrop"><div class="gamaExcelIcon">🖼️</div><b data-gi=c3f8ff5acbf2>Selecciona o arrastra las fotos de tus productos</b><span data-gi=e9250d978d8e>Formatos compatibles: .jpg, .png y .webp — puedes seleccionar varias a la vez</span><input id="gamaPhotoFiles" type="file" accept="image/*" multiple></div><div class="gamaPhotoHint" data-gi=0107e8f8d234>El nombre del archivo puede contener la <b data-gi=ba93fb125822>referencia, el nombre del producto o ambos</b>: <code>SKU-001.jpg</code>, <code>Tornillo hexagonal.png</code> o <code>SKU-001 Tornillo hexagonal frente.jpg</code>. Mayúsculas, acentos, espacios, guiones y guiones bajos se reconocen. Si hay varios candidatos, añade la referencia y el nombre; las coincidencias ambiguas no se importan.</div><div id="gamaPhotoStatus" class="gamaExcelInfo" data-gi=d674f2920b4c>Ninguna foto seleccionada.</div><label class="gamaPhotoOpt"><input type="checkbox" id="gamaPhotoKeep" data-gi=32d46931ce14> Conservar las fotos que ya existen (no reemplazarlas)</label><div id="gamaPhotoPreview" class="gamaExcelPreview" data-gi=06f5ea0fcb93>Selecciona fotos para comprobar la correspondencia con tus productos.</div><div class="gamaExcelActions"><button type="button" id="gamaPhotoImport" class="primary" disabled data-gi=dd9cf954111a>Importar fotos</button></div></div><div class="gamaExcelBody" id="gamaExcelPanelOptimize" hidden><div class="gamaPhotoHint" data-gi=77d1ab5a2a1a>Vuelve a guardar las fotos ya subidas a un tamaño ajustado a la pantalla (<b>320 px</b>). La aplicación nunca las enseña más grandes, así que no se nota la diferencia, pero pesan mucho menos y la base de datos gasta menos tráfico. Las fotos que ya sean pequeñas se dejan como están.</div><div id="gamaOptStatus" class="gamaExcelInfo" data-gi=43ec50213d03>Pulsa el botón para revisar las fotos guardadas.</div><div id="gamaOptResult" class="gamaExcelPreview" data-gi=0976200111c1>Todavía no se ha optimizado nada.</div><div class="gamaExcelActions"><button type="button" id="gamaOptRun" class="primary" data-gi=d3f1025271c5>Optimizar las fotos guardadas</button></div></div>`;bind();}
...
function photoPreview(){
 ...
 p.innerHTML='<table><thead><tr><th></th><th data-gi=75d6b4579e1d>Archivo</th><th data-gi=10ddff5fcc6f>Referencia</th><th data-gi=77b9238931ed>Producto</th><th data-gi=98e5acddb6c4>Estado</th></tr></thead><tbody>'
...
async function importRows(){
 const btn=document.getElementById('gamaExcelImport'),st=document.getElementById('gamaExcelStatus');btn.disabled=true;
 const api=window.GamaCloud;if(!api){alert('La conexión con la nube de Architect ERP no está disponible.');btn.disabled=false;return}
 st.textContent='Comprobando duplicados…';
 try{
  if(state.type==='customerPrices'){await importarTarifas(api,state.rows,st);return}
  const seen=await existingKeys(api,state.type);let ok=0,fail=0,dup=0;const errors=[];
  for(const [index,row] of state.rows.entries()){
   try{
    let r,key;
    if(state.type==='clients'||state.type==='suppliers'){
     if(!row.name)throw Error('El nombre es obligatorio');
     key=dupKey(row.name,row.address);if(seen.has(key)){dup++;continue}
    }
    if(state.type==='products'){
     if(!row.name)throw Error('El nombre es obligatorio');
     r=await api.insert('products',row);
    }else if(state.type==='clients')r=await api.insert('customers',{name:row.name,identification:row.tax_id||null,address:row.address||null,city:row.city||null,province:row.province||null,phone:row.phone||null,email:row.email||null,notes:row.notes||null,active:true});
    else if(state.type==='suppliers')r=await api.insert('suppliers',{name:row.name,tax_id:row.tax_id||null,address:row.address||null,city:row.city||null,contact_name:row.contact_name||null,phone:row.phone||null,email:row.email||null,notes:row.notes||null,active:true});
    if(r?.error)throw r.error;
    if(!r)throw Error('Tipo de importación no disponible');
    if(key)seen.add(key);ok++;
   }catch(e){
    if(e.code==='23505')dup++;else fail++;
    if(errors.length<10)errors.push('Fila '+(index+2)+': '+(e.code==='23505'?'Producto duplicado. ':'')+(e.message||e));
   }
  }
  st.textContent=`Importación finalizada: ${ok} fila(s) importada(s), ${dup} duplicada(s) omitida(s), ${fail} error(es).`+(errors.length?' Detalles: '+errors.join(' | '):'');
 }catch(e){st.textContent='No se pudo completar la importación: '+(e.message||e)}finally{btn.disabled=false}
}
