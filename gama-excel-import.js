...
(function(){
  ...
  function loadXLSX(){
    ...
    return new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=XLSX_URL;s.onload=()=>window.XLSX?resolve(window.XLSX):reject(new Error('XLSX no disponible'));s.onerror=()=>reject(new Error('No se pudo cargar el motor de Excel'));document.head.appendChild(s)})
  }
  async function readFile(file){
    if(!file)return;if(file.size>10*1024*1024){alert('El archivo supera el límite de 10 MB.');return}
    try{const lib=await loadXLSX();const wb=lib.read(await file.arrayBuffer(),{type:'array',cellDates:true});state.file=file;state.workbook=wb;state.sheet=pickSheet(wb,state.type);const r=rowsFromSheet(wb,state.sheet);state.headers=r.headers;state.rows=r.rows;render()}catch(e){alert('Archivo Excel inválido: '+e.message)}}
  ...
  function render(){
    ...
    const types=[['products','Productos','Referencias, precios, stock',''],['clients','Clientes','Contactos y datos','client'],['suppliers','Proveedores','Contactos y condiciones','supplier']];
    const has=!!state.file;
    s.innerHTML=`<div class="card"><div class="gxiHead"><div><h2>📊 Importar Excel</h2><p>Importa tus <b>productos, clientes y proveedores</b> directamente desde un archivo Excel.</p></div><span class="gxiBadge">.xlsx · .xls · máx. 10 MB</span></div>
      <div class="gxiSteps"><div class="gxiStep active">1 · Selección</div><div class="gxiStep ${has?'active':''}">2 · Vista previa</div><div class="gxiStep">3 · Importación</div><div class="gxiStep">4 · Resultados</div></div>
      <div class="gxiGrid"><div class="gxiCard"><h3 style="margin-top:0">¿Qué deseas importar?</h3><div class="gxiTypes">${types.map(t=>`<button type="button" class="gxiType ${t[3]} ${state.type===t[0]?'active':''}" data-gxi-type="${t[0]}"><div class="ico">${icon(t[0])}</div><b>${t[1]}</b><small style="display:block;color:#71808a;margin-top:4px">${t[2]}</small></button>`).join('')}</div>
      <div class="gxiDrop" id="gxiDrop" style="margin-top:12px"><div style="font-size:35px">☁️</div><strong>${has?esc(state.file.name):'Arrastra y suelta tu archivo Excel aquí'}</strong><small>${has?'Archivo cargado · '+state.rows.length+' fila(s)':'o haz clic para seleccionar un archivo'}</small><label class="gxiFile">📁 Elegir archivo<input id="gxiFile" type="file" accept=".xlsx,.xls" hidden></label></div>
      <div class="gxiHelp"><b>Consejo:</b> para una importación sin errores, usa una hoja por tipo: <b>Productos</b>, <b>Clientes</b> y <b>Proveedores</b>. Las columnas se reconocen aunque estén en español o en inglés.</div></div>
      <div class="gxiCard"><h3 style="margin-top:0">Hoja de Excel</h3><select id="gxiSheet" ${!state.workbook?'disabled':''}>${(state.workbook?.SheetNames||[]).map(n=>`<option ${n===state.sheet?'selected':''}>${esc(n)}</option>`).join('')}</select><p class="gxiInfo"><b>Columnas obligatorias</b><br>${required(state.type).map(k=>`• ${k==='reference'?'Referencia':k==='name'?'Nombre':'Nombre / razón social'}`).join('<br>')}<br><br><b>Detección automática</b><br>Referencia, nombre, categoría, precio, stock, correo, teléfono, dirección, IVA/RUC, contacto, etc.</p><div id="gxiMap"></div></div></div>
      ${has?previewHtml():''}<div class="gxiActions"><button class="primary" id="gxiImport" ${has&&state.rows.length?'':'disabled'}>⬆ Importar ${state.rows.length||''} fila(s)</button><button class="secondary" id="gxiReset">Restablecer</button></div><div id="gxiResult"></div></div>`;
  ...
  function previewHtml(){
    ...
    return `<div class="gxiCard" style="margin-top:12px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><h3 style="margin:0">Vista previa</h3><span class="gxiBadge">${state.rows.length} fila(s) · hoja «${esc(state.sheet)}»</span></div><div class="gxiStats"><div class="gxiStat"><small>Filas</small><b>${state.rows.length}</b></div><div class="gxiStat"><small>Columnas</small><b>${state.headers.length}</b></div><div class="gxiStat"><small>Obligatorios OK</small><b>${missing.length?'No':'Sí'}</b></div><div class="gxiStat"><small>Hoja</small><b>${esc(state.sheet)}</b></div></div>${missing.length?`<div class="gxiResult gxiBad">No se encontraron las columnas obligatorias: ${missing.map(esc).join(', ')}.</div>`:''}<div class="gxiPreview"><table><thead><tr>${keys.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${preview.map(r=>`<tr>${keys.map(k=>`<td>${esc(r[k])}</td>`).join('')}</tr>`).join('')}</tbody></table></div></div>`
  }
  async function importRows(){
    const missing=required(state.type).filter(k=>!findHeader(state.headers,k));if(missing.length){alert('No se puede importar: faltan columnas obligatorias.');return}
    const result={inserted:0,skipped:0,errors:[],cloud:0};const rows=state.rows.map(normalizedRow);
    if(state.type==='suppliers'){
      const arr=load(SUP_KEY);const seen=new Set(arr.map(x=>norm(x.name)));rows.forEach((r,i)=>{const name=String(r.name||'').trim();if(!name){result.skipped++;result.errors.push('Fila '+(i+2)+': nombre del proveedor vacío');return}if(seen.has(norm(name))){result.skipped++;return}arr.push({id:uid(),name,tax:String(r.tax||''),contact:String(r.contact||''),phone:String(r.phone||''),email:String(r.email||''),city:String(r.city||''),address:String(r.address||''),notes:String(r.description||r.payment||'')});seen.add(norm(name));result.inserted++});save(SUP_KEY,arr);
    }else if(state.type==='clients'){
      const arr=load(CLIENT_KEY);const seen=new Set(arr.map(x=>norm(x.email||x.name)));rows.forEach((r,i)=>{const name=String(r.name||'').trim();if(!name){result.skipped++;result.errors.push('Fila '+(i+2)+': nombre del cliente vacío');return}const key=norm(r.email||name);if(seen.has(key)){result.skipped++;return}arr.push({id:uid(),name,email:String(r.email||''),phone:String(r.phone||''),address:String(r.address||''),city:String(r.city||''),tax:String(r.tax||''),notes:String(r.description||r.payment||'')});seen.add(key);result.inserted++});save(CLIENT_KEY,arr);
    }else{
      ...
      for(let i=0;i<rows.length;i++){const r=rows[i],name=String(r.name||'').trim(),reference=String(r.reference||'').trim(),barcode=String(r.barcode||reference).trim();if(!name||!reference){result.skipped++;result.errors.push('Fila '+(i+2)+': falta la referencia o el nombre');continue}const key=norm(barcode||reference||name);if(seen.has(key)){result.skipped++;continue}const p={id:uid(),reference,name,barcode,category:String(r.category||''),unit:String(r.unit||''),purchasePrice:Number(String(r.purchasePrice||'').replace(',','.'))||0,salePrice:Number(String(r.salePrice||'').replace(',','.'))||0,stock:Number(String(r.stock||'').replace(',','.'))||0,minStock:Number(String(r.minStock||'').replace(',','.'))||0,description:String(r.description||'')};db.products.push(p);seen.add(key);result.inserted++;if(cloud&&typeof cloud.createProduct==='function'){try{await cloud.createProduct(p);result.cloud++}catch(e){}}}
  ...
  function renderResult(r){const el=$('gxiResult');if(!el)return;el.innerHTML=`<div class="gxiResult gxiGood"><b>Importación finalizada.</b> ${r.inserted} fila(s) importada(s), ${r.skipped} omitida(s).${r.cloud?' Sincronización en la nube: '+r.cloud+'.':''}</div>${r.errors.length?`<div class="gxiResult gxiWarn"><b>Para revisar:</b><br>${r.errors.slice(0,12).map(esc).join('<br>')}${r.errors.length>12?'<br>…':''}</div>`:''}`}
  function replaceEmptyMenu(){
    ...
    const title=target.querySelector('.gamaApp b,.gamaF2Title,.gamaMenuTitle');if(title)title.textContent='Importar Excel';
