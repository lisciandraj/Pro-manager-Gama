/* GAMA Module Loader — Excel + Achats */
(function(){'use strict';
function loadScript(src){return new Promise(function(resolve,reject){if(document.querySelector('script[data-gama-src="'+src+'"]'))return resolve();var s=document.createElement('script');s.src=src;s.dataset.gamaSrc=src;s.onload=resolve;s.onerror=reject;document.head.appendChild(s);});}
function mount(){
...
function openExcel(){
  document.querySelectorAll('section').forEach(function(s){s.classList.remove('active');if(s.id!=='gama-excel-import-section')s.style.display='none';});
  const sec=document.getElementById('gama-excel-import-section');if(sec){sec.style.display='block';sec.classList.add('active');}
  document.getElementById('mainmenu')?.setAttribute('hidden','');mount();window.scrollTo({top:0,behavior:'smooth'});
}
...
function installPurchasesButton(){
 const host=document.querySelector('.tabs');if(!host||host.querySelector('[data-gama-purchases]'))return;
 const b=document.createElement('button');b.type='button';b.className='tab';b.dataset.gamaPurchases='1';b.innerHTML='🛒<span>Achats</span>';b.onclick=function(){if(window.gamaShowPurchases)window.gamaShowPurchases();};host.appendChild(b);
}
async function boot(){
  try{mount();installExcelButton();
    if(!window.db)window.db={};
    await loadScript('gama-purchases-v13.js?v=20260827-1');
    installPurchasesButton();
  }catch(e){console.warn('[GAMA] Module loader:',e);}
  new MutationObserver(function(){mount();installExcelButton();installPurchasesButton();}).observe(document.body,{subtree:true,childList:true});
}
