...
async function label(d,f,p){try{await window.GamaCompany?.load(true)}catch(e){window.gamaToast?.(err(e));return}const theme=window.GamaPdfTemplate.theme(),html=`<div style="display:flex;align-items:center;gap:16px;border-bottom:2px solid ${theme.profile.document_secondary||"#087C8B"};padding-bottom:8px">${theme.logo?`<img alt="${esc(theme.profile.legal_name||"Architect ERP")}" src="${esc(theme.logo)}" style="width:90px;height:60px;object-fit:contain">`:""}<div><h2 style="color:var(--arc-text);margin:0" data-gi=8e9578c91c93>ETIQUETA DE BULTO</h2><p>${esc(theme.profile.legal_name||"Architect ERP")} · ${esc(d.order.number)}</p></div></div><p>${esc(d.order.customer_name)}<br>${esc(d.order.delivery_address)}</p>${barcode(p.barcode)}<p>${esc(packLabel(d,f,p))}</p><ul>${f.package_lines.filter(x=>x.package_id===p.id).map(x=>{const pl=f.pick_lines.find(l=>l.id===x.pick_line_id);return `<li>${esc(d.lines.find(l=>l.id===pl?.order_line_id)?.product_name)} × ${n(x.quantity)}</li>`}).join('')}</ul>`;
