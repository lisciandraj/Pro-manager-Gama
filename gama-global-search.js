...
function mount(input){
 input.placeholder=t('placeholder');input.setAttribute('aria-label',t('placeholder'));input.setAttribute('aria-haspopup','dialog');input.setAttribute('autocomplete','off');input.setAttribute('data-gi-ignore','');input.maxLength=160;input.dataset.gamaSearchTrigger='1';
...
function ensure(){
 ...
 window.ArcUI.render(dialog,`<h2 id="gamaSpotlightTitle" class="gspSr"></h2><div class="gspSearch">${icon}<input id="gspInput" type="search" maxlength="160" autocomplete="off" role="combobox" aria-autocomplete="list" aria-controls="gspList" aria-expanded="false"><button class="arcButton" id="gspClose" type="button">×</button></div><div class="gspBody"><div id="gspHint"></div><div id="gspStatus" role="status" aria-live="polite"></div><div id="gspList" role="listbox"></div><div id="gspExtra"></div></div><footer id="gspFooter"></footer>`);
...
function close(){closing=true;sessionEpoch++;if(dialog?.open)dialog.close();if(returnFocus?.dataset?.gamaSearchTrigger){returnFocus.value='';returnFocus.blur()}else returnFocus?.focus?.();queueMicrotask(()=>{closing=false});}
...
function render(){
 ...
 window.ArcUI.render($('gspList'),sorted.filter(g=>g.items.length).map(g=>`<div role="group" aria-label="${esc(t(g.source))}"><div class="gspGroup">${esc(t(g.source))}<span>${g.items.length}</span></div>${[...g.items].sort((a,b)=>b.score-a.score||a.title.localeCompare(b.title)).map(r=>{const index=visible.push(r)-1;return `<div role="option" tabindex="-1" id="gspOption${index}" data-index="${index}" aria-selected="false"><span class="gspIcon">${esc(marks[r.source]||'•')}</span><span class="gspText"><strong>${esc(r.title)}</strong><small>${esc(r.subtitle)}${r.archived?' · '+esc(t('archived')):''}</small></span>${r.balance!=null?`<span class="gspAmount">${esc(t('balance'))}<b>${esc(money(r.balance))}</b></span>`:r.total!=null?`<span class="gspAmount"><b>${esc(money(r.total))}</b></span>`:''}<span class="gspArrow" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M7 17 17 7M9 7h8v8"/></svg></span></div>`}).join('')}</div>`).join(''));
 ...
 window.ArcUI.render($('gspExtra'),(q.length<2?`<div class="gspSuggestions">${t('suggestions').filter(s=>!profile||Core.selectedSources(Core.parse(s),context()).length).map(s=>`<button class="arcButton" type="button" data-suggestion="${esc(s)}">${esc(s)}</button>`).join('')}</div>`:'')+(errors.length?`<p class="gspError">${esc(t('error'))} (${errors.map(g=>esc(t(g.source))).join(', ')})</p><button class="arcButton" type="button" id="gspRetry">${esc(t('retry'))}</button>`:'')+(more.length?`<p class="gspHint">${esc(t('partial'))}</p><div class="gspMore">${more.map(g=>`<button class="arcButton" type="button" data-more="${esc(g.source)}" ${busy?'disabled':''}>${esc(g.items.length?t('more'):t('scan'))} · ${esc(t(g.source))}</button>`).join('')}</div>`:''));
...
async function preview(r,c,signal){
 ...
 window.ArcUI.render(d,`<div class="gspRecordHead"><h2 id="gspRecordTitle">${esc(r.title)}</h2><button class="arcButton" type="button" data-close aria-label="${esc(t('close'))}">×</button></div><dl>${Object.entries(data).filter(([key,v])=>key!=='id'&&v!=null&&v!=='').map(([key,v])=>`<div><dt>${esc(labels[key]||key)}</dt><dd>${esc(typeof v==='boolean'?(v?'✓':'—'):v)}</dd></div>`).join('')}</dl><div class="gspRecordActions"><button class="arcButton" data-back>${esc(t('back'))}</button><button class="arcButton" data-module>${esc(t('open'))}</button></div>`);
