...
function slice(key,rows,accessors){let r=Array.isArray(rows)?rows:[];if(accessors&&window.GamaSort)r=GamaSort.apply(key,r,accessors);const p=clamp(key,r.length);return r.slice(p*SIZE,p*SIZE+SIZE)}
