...
function setBarcode(value){
 ...
 const apply=()=>{const el=document.getElementById('pBarcode');if(!el)return;if(setter)setter.call(el,v);else el.value=v};
 apply();
...
async function nativeScan(video,status){
 ...
 try{
  let fs=['ean_13','ean_8','upc_a','upc_e','code_128','code_39','code_93','itf','qr_code','data_matrix'];
  const supported=await BarcodeDetector.getSupportedFormats();fs=fs.filter(x=>supported.includes(x));if(!fs.length)return false;
  const detector=new BarcodeDetector({formats:fs});
  const loop=async()=>{if(!overlay)return;try{if(video.readyState>=2){const found=await detector.detect(video);const code=found?.[0]?.rawValue;if(code){setBarcode(code);close();return}}}catch(e){}raf=requestAnimationFrame(loop)};
  status.textContent='Apunte la cámara al código de barras';raf=requestAnimationFrame(loop);return true;
...
async function openProductScanner(){
 ...
 try{
  if(!navigator.mediaDevices?.getUserMedia)throw Error('camera');
  stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'},width:{ideal:1280},height:{ideal:720}},audio:false});
  video.srcObject=stream;await video.play();
  if(await nativeScan(video,status))return;
  status.textContent='Preparando lector…';
  const ZX=await loadZXing();const r=new ZX.BrowserMultiFormatReader();
  readerControls=await r.decodeFromStream(stream,video,result=>{const code=result?.getText?.()||result?.text||result?.rawValue||'';if(code){setBarcode(code);close()}});
