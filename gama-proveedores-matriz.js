...
function listSuppliers(){
 ...
 host.innerHTML=data.length?GamaPage.slice('suppliersDir',data).map(x=>`<div style="padding:12px 0;border-bottom:1px solid #edf1f2"><div style="display:flex;justify-content:space-between;gap:8px"><b>${esc(x.name)}</b><button class="danger gamaPMIconBtn" data-del="${esc(x.id)}">Eliminar</button></div><div class="muted">${esc(x.tax||'Sin RUC')} · ${esc(x.city||'Sin ciudad')}</div><div style="margin-top:5px">${esc(x.contact||'')} ${x.phone?'· '+esc(x.phone):''}</div><div class="muted">${esc(x.email||'')}</div><div style="margin-top:5px">📍 ${esc(x.address||'Sin dirección')}</div>${x.notes?`<div class="notice" style="margin-top:7px">${esc(x.notes)}</div>`:''}</div>`).join('')+GamaPage.controls('suppliersDir',data.length):'<div class="gamaPMEmpty">No hay proveedores registrados.</div>';
