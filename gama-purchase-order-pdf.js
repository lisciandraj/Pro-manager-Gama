...
function build(o){
 ...
 function columns(){l.room(16);doc.setFillColor(...window.GamaPdfTemplate.secondary);doc.rect(14,l.y-5,182,9,'F');doc.setTextColor(...window.GamaPdfTemplate.onSecondary);doc.setFontSize(8);doc.setFont('helvetica','bold');[['Producto / referencia',16],['Cant.',119],['P. compra',140],['Subtotal',177]].forEach(([v,x])=>doc.text(v,x,l.y));l.y+=10}
 columns();
 (o.items||[]).forEach(x=>{
  ...
  while(lines.length){
   ...
   if(first){doc.text(String(x.qty),131,l.y,{align:'right'});doc.text(GamaCurrency.symbol(o.currency)+Number(x.cost||0).toFixed(2),166,l.y,{align:'right'});doc.text(GamaCurrency.symbol(o.currency)+(Number(x.qty||0)*Number(x.cost||0)).toFixed(2),194,l.y,{align:'right'});first=false}
   l.y+=chunk.length*4+5;
  }
  doc.setDrawColor(224,232,236);doc.setLineWidth(.2);doc.line(14,l.y-3,196,l.y-3);
 });
 l.room(20);l.y+=4;l.text('TOTAL: '+GamaCurrency.symbol(o.currency)+Number(o.total||0).toFixed(2),14,true);
