...
async function cloudSupplier(s){try{const C=window.GamaCloud;if(!C)return null;const db=await C.db();const q=await db.from('suppliers').select('*').eq('name',s.name).limit(1);if(q.data?.[0])return q.data[0];const {legacy_id,...row}=s;const r=await C.insert('suppliers',row);if(r.error)console.warn('[GAMA supplier bridge]',r.error);return r.data||null}catch(e){console.warn('[GAMA supplier bridge]',e);return null}}
window.gamaResolveSupplierId=async function(value){if(!value||!value.startsWith('legacy:'))return value;const name=decodeURIComponent(value.slice(7));const s=localSuppliers().find(x=>x.name===name);if(!s)return null;const saved=await cloudSupplier(s);return saved?.id||null};
