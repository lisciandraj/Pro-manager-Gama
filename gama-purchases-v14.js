...
(function(){
  ...
  window.gamaAssignReplenishmentSupplier=async function(button){
    if(!canOrder())return;
    const group=button.closest('[data-replenishment-group]'),select=group?.querySelector('[data-replenishment-supplier]'),sid=select?.value;
    if(!sid)return alert('Selecciona un proveedor.');
    const unassigned=needs.filter(n=>!n.supplier_id&&Number(n.suggested_purchase)>0);
    if(!unassigned.length)return;
    if(!confirm('Asignar este proveedor a '+unassigned.length+' producto(s) sin proveedor?'))return;
    button.disabled=true;
    let ok=0,fail=0;
    for(const n of unassigned){
      try{const r=await C().update('products',n.product_id,{supplier_id:sid});if(r.error)throw r.error;ok++}catch(e){console.error('[Coco supplier assignment]',n.product_id,e);fail++}
    }
    await load();
    msg(ok+' producto(s) asignado(s) a '+supplierName(sid)+'.'+(fail?' '+fail+' error(es).':''),fail===0);
  };
