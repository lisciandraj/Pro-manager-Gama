...
(function(){
  ...
  function inject(){
    ...
    const s=document.createElement('section');s.id='gamaPurchasesV14';window.ArcUI.render(s,`
      ...
      <div class="gp14Grid"><div class="arcPanel card gp14Form"><div class="gp14Title"><h3 data-gi=78205ea91be9>Nuevo pedido a proveedor</h3><span class="gp14Role" id="gp14Role"></span></div>
      ...
      <button class="arcButton secondary gp14Full" id="gp14Add" data-gi=93f051a1deca>＋ Añadir producto</button><div id="gp14Draft" class="gp14DraftBox"></div>
  ...
  function renderDraft(){const h=$('gp14Draft');if(!draft.length){window.ArcUI.render(h,'<div class="muted" data-gi=e708556f30d1>No hay productos en el pedido.</div>');return}window.ArcUI.render(h,draft.map((l,i)=>{const p=products.find(x=>x.id===l.product_id);return `<div class="gp14DraftLine"><div><b>${esc(p?.name||'Producto')}</b><small>${esc(p?.reference||p?.barcode||'')}</small></div><span>${l.quantity}</span><span>${money(l.quantity*l.unit_cost)}</span><button class="arcButton gp14X" onclick="window.gamaRemovePurchaseLineV14(${i})">×</button></div>`}).join(''))}
