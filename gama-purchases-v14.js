...
(function(){
  ...
  function inject(){
    ...
    const s=document.createElement('section');s.id='gamaPurchasesV14';window.ArcUI.render(s,`
      ...
      <label>Impuesto (%)</label><input id="gp14Tax" type="number" min="0" max="100" step="0.001" value="0"><button class="arcButton secondary gp14Full" id="gp14Add" data-gi=93f051a1deca>＋ Añadir producto</button><div id="gp14Draft" class="gp14DraftBox"></div>
  ...
  async function saveOrder(){
   ...
   try{
    ...
    else saved=await window.ArcData.rpc('gama_purchase_save',{p_data:{...payload,request_key:saveRequest.key}});
  ...
  window.addEventListener('gama:auth-change',()=>{receiptCommands.clear();projectContext=null;projectSaveKey=null;draft=[];$('gp14ProjectContext')?.remove()});
