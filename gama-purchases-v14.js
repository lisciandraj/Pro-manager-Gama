...
(function(){
  ...
  async function receive(id){if(!canReceive())return alert('Tu perfil no puede registrar recepciones.');const o=orders.find(x=>x.id===id);if(!o||!['sent','partial'].includes(o.status))return;const ls=lines.filter(x=>x.purchase_order_id===id);const payload=[];for(const l of ls){const pending=Math.max(0,Number(l.quantity)-Number(l.received_quantity));if(!pending)continue;const input=$('gp14Recv_'+l.id);const q=Number(input?.value||0);if(q<0||q>pending)return detailMsg('Una cantidad recibida no es válida.');if(q>0)payload.push({line_id:l.id,quantity:q})}if(!payload.length)return detailMsg('Introduce al menos una cantidad para recibir.');const comment=$('gp14ReceiveComment')?.value.trim()||null;const client=await C().db();const r=await client.rpc('gama_receive_purchase',{p_purchase_order_id:id,p_lines:payload,p_comment:comment});if(r.error){detailMsg(mapRpcError(r.error.message));return}await load();if(selectedId===id){await openOrder(id);detailMsg('Recepción registrada: inventario actualizado automáticamente.',true)}}
