...
function updateProductInfo(el,code){
...
function fill(code){const el=getTarget(targetId),v=String(code??'').trim();if(!el||!v)return false;const ok=persistValue(el,v);if(ok)updateProductInfo(el,v);close();return ok;}
...
window.scan=scan;
function bind(){style();document.querySelectorAll('button').forEach(b=>{if(!norm(b.textContent).includes('escanear'))return;const input=b.closest('.scanner')?.querySelector('input')||getTarget();if(!input)return;b.classList.add('gamaPhoneScanBtn');b.type='button';b.onclick=null;if(!b.dataset.gamaScannerBound){b.dataset.gamaScannerBound='1';b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();start(input.id)},true)}})}
