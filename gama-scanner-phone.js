...
function style(){if(document.getElementById('gamaScannerPhoneStyle'))return;const s=document.createElement('style');s.id='gamaScannerPhoneStyle';s.textContent='.gamaPhoneScanBtn{background:var(--arc-accent-600)!important;color:#fff!important;border-color:var(--arc-accent-600)!important}.gamaPhoneScanBtn:active{transform:scale(.98)}#gamaPhoneScanner{padding-bottom:max(18px,env(safe-area-inset-bottom))!important;position:fixed;inset:0;z-index:100000;background:var(--arc-text);display:flex;flex-direction:column;padding:18px;box-sizing:border-box;color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}#gamaPhoneScanner .h{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}#gamaPhoneScanner .title{font-size:18px;font-weight:800}#gamaPhoneScanner .close{border:0;background:#fff;color:var(--arc-text);border-radius:12px;padding:10px 14px;font-weight:800}#gamaPhoneScanner .video{position:relative;flex:1;min-height:80px;overflow:hidden;border-radius:18px;background:#000}#gamaPhoneScanner video{width:100%;height:100%;object-fit:cover}#gamaPhoneScanner .frame{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(78vw,420px);height:170px;border:3px solid var(--arc-success);border-radius:18px;box-shadow:0 0 0 9999px rgba(0,0,0,.28)}#gamaPhoneScanner .recovery{display:flex;gap:8px;flex-wrap:wrap;padding-top:12px}#gamaPhoneScanner .recovery button{flex:1;min-height:44px;font-size:16px}#gamaPhoneScanner .help{font-size:14px;margin:10px 0}#gamaPhoneScanner .status{text-align:center;font-size:14px;font-weight:700;padding:14px 6px 6px}';document.head.appendChild(s)}
let zxingLoad=null;
...
async function start(id){
 ...
 overlay=document.createElement('div');overlay.id='gamaPhoneScanner';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-label','Escanear código de barras');overlay.setAttribute('data-gi-aria-label','live');
 overlay.innerHTML='<div class="h"><div class="title" data-gi=4791a6ffaf10>Escanear código de barras</div><button class="close" type="button" data-gi=aeccae342e4b>Cerrar</button></div><div class="video"><video playsinline webkit-playsinline muted autoplay></video><div class="frame"></div></div><div class="status" role="status" aria-live="polite" data-gi=e6013f70ce82>Activando cámara…</div><p class="help" hidden></p><a class="diagnostic" href="./camera-check.html" hidden style="color:#fff;padding:8px 0" data-gi=5ccd0440945f>Probar cámara fuera del ERP</a><div class="diagnosticCode" hidden style="font-size:12px;overflow-wrap:anywhere"></div><div class="recovery"><button type="button" class="retry" data-gi=9984cd2d2ebd>Reintentar cámara</button><button type="button" class="manual" data-gi=08e635b52494>Introducir código</button></div>';
