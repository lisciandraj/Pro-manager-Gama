/* GAMA — Cabecera estándar + navegación robusta para todos los módulos */
(function(){
  'use strict';
  const STYLE='gamaStandardUIStyleV4';
  ...
  const TITLES={
    ...
    movement:['🔄 Movimientos','Entradas, salidas y ajustes de inventario'],
    ...
    backup:['💾 Copias de seguridad','Exportación y restauración de datos'],
    barcode:['🏷️ Códigos de barras','Generación e impresión de etiquetas']
  };
  function menu(){
  ...
  function directStandardHeader(sec){return sec.querySelector(':scope > .gamaStdHeader, :scope > [data-gama-standard-header]')}
  function directOwnHeader(sec){return sec.querySelector(':scope > .gp14Head')}
  function addStandardHeader(sec){
    if(SKIP.has(sec.id)||directStandardHeader(sec))return;
    if(directOwnHeader(sec)){
      ...
      if(!existing.querySelector('.gamaStdBack')){
        ...
        const b=document.createElement('button');b.type='button';b.className='gamaStdBack';b.textContent='← Volver al menú';b.setAttribute('aria-label','Volver al menú');b.onclick=menu;actions.appendChild(b);
      }
      return;
    }
    const title=titleFor(sec),header=document.createElement('div');
    header.className='gamaStdHeader';header.setAttribute('data-gama-standard-header','1');
  ...
  /* Corrige el bug de navegación : showTab ajoutait seulement .active alors que
     le bouton retour masquait les sections avec style.display='none'. */
  function patchShowTab(){
    if(typeof window.showTab!=='function'||window.showTab.__gamaPatched)return false;
    const original=window.showTab;
    function fixedShowTab(id,btn){
      document.querySelectorAll('section').forEach(s=>{s.classList.remove('active');s.style.display='none'});
      const target=document.getElementById(id);
      if(target){target.style.display='block';target.classList.add('active')}
      const back=document.getElementById('globalBack');if(back)back.style.display='none';
      document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));
      try{if(typeof window.renderAll==='function')window.renderAll()}catch(e){console.warn('[GAMA renderAll]',e)}
      window.scrollTo({top:0,behavior:'smooth'});
      setTimeout(standardize,0);
    }
    fixedShowTab.__gamaPatched=true;
    window.showTab=fixedShowTab;
    return true;
  }
  function boot(){
    standardize();
    patchShowTab();
    let tries=0;const timer=setInterval(()=>{if(patchShowTab()||++tries>200)clearInterval(timer)},50);
    new MutationObserver(()=>{standardize();patchShowTab()}).observe(document.body,{childList:true,subtree:true});
  }
