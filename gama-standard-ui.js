...
(function(){
  'use strict';
  const STYLE='gamaStandardUIStyleV2';
  ...
  function hasOwnHeader(sec){return !!sec.querySelector(':scope > .gamaStdHeader, :scope > .gp14Head, :scope > [data-gama-standard-header]');}
  function standardize(){
    addStyles();
    document.querySelectorAll('section').forEach(sec=>{
      if(SKIP.has(sec.id)||sec.dataset.gamaStandard==='1'||!sec.firstElementChild)return;
      if(hasOwnHeader(sec)){
        const existing=sec.querySelector(':scope > .gp14Head');
        if(existing&&!existing.querySelector('.gamaStdBack')){
          const actions=existing.querySelector('.gp14TopActions')||existing;
          const b=document.createElement('button');b.type='button';b.className='gamaStdBack';b.textContent='← Volver al menú';b.onclick=menu;actions.appendChild(b);
        }
        sec.dataset.gamaStandard='1';return;
      }
      ...
      sec.insertBefore(header,sec.firstElementChild);sec.dataset.gamaStandard='1';
    });
  }
