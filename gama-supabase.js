/* GAMA V11 — central Supabase data layer */
(function () {
  'use strict';
  ...
  async function init(){
    if(!SUPABASE_ANON_KEY){emit('gama:cloud-status',{ready:false,configured:false,reason:'missing_public_key'});return null}
    if(client)return client;
    const sb=await loadClient();
    client=sb.createClient(SUPABASE_URL,SUPABASE_ANON_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});
  ...
  window.GamaCloudReady=init().then(function(){const s=document.createElement('script');s.src='gama-cloud-products.js?v=11';s.async=true;document.head.appendChild(s);return window.GamaCloud});
})();