...
export function createHandler({env,fetch:fetcher}){
 ...
 const text=await req.text();if(text.length>3000)return reply({error:'PAYLOAD_TOO_LARGE'},413);const data=JSON.parse(text);
 ...
 const invited=await request('/auth/v1/invite?redirect_to='+encodeURIComponent(origin+'/Pro-manager-Gama/?setup=password'),{method:'POST',admin:true,body:{email,data:{full_name:name}}});
