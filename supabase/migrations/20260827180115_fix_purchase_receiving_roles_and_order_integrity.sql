create unique index if not exists purchase_orders_order_number_uidx on public.purchase_orders(order_number);
...
begin
  ...
  for v_line in select * from jsonb_array_elements(coalesce(p_lines,'[]'::jsonb)) loop
    ...
    insert into public.stock_movements(product_id,type,quantity,reason,comment,user_id)
      values(v_product.id,'in',v_qty,'Recepción de compra',coalesce(p_comment,'Réception '||v_po.order_number),auth.uid());
  end loop;
...
revoke all on function public.gama_receive_purchase(uuid,jsonb,text) from public;
