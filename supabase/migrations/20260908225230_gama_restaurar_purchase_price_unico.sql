...
-- Vuelve el precio de compra único, recuperando el valor que se había
-- trasvasado a price_a.
alter table public.products
  add column if not exists purchase_price numeric not null default 0;

update public.products
   set purchase_price = price_a
 where price_a is not null and purchase_price = 0;

alter table public.products
  add constraint products_purchase_price_check check (purchase_price >= 0);

-- La recepción de mercancía vuelve a registrar el último coste pagado.
