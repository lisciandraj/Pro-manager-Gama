...
begin
 ...
 if u is not null then
  ...
  if not exists(select 1 from public.gama_document_references where table_name='customer_requests' and document_id=fresh and document_reference like 'DEM-%') then raise exception 'New request trigger failed';end if;
 end if;
 ...
 if (select document_reference from public.gama_document_references where document_id=s2)<>'EXP-B-'||lpad(num::text,8,'0') then raise exception 'Partial shipment reference mismatch';end if;
 perform private.gama_register_document('sales_deliveries',jsonb_build_object('id',s1,'order_id',o));
