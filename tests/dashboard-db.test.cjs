...
test('company dashboard aggregates periods without fan-out, respects RLS and access profiles',async()=>{
 ...
 await db.exec(`reset role;set session_replication_role=replica;insert into return_orders(id,number,kind,customer_id,order_id,invoice_id,reason,status) values('${id(9940)}','DEV-00009940','customer','${customer}','${order}','${invoice}','damaged','processed');insert into return_credits(number,return_id,invoice_id,amount,issued_on) values('NCR-00009941','${id(9940)}','${invoice}',5,'2020-02-15'),('NCR-00009942','${id(9940)}','${invoice}',7,'2020-02-16');insert into financial_accounts(name,kind,currency,opening_balance) values('USD bank','bank','USD',100),('EUR bank','bank','EUR',200);set session_replication_role=origin;`);
