...
test('tracks internal/external invoice once, partial and voided payments, overdue balance and full settlement',async({page})=>{
 ...
 await page.evaluate(()=>__DB.external_invoice_payments[1].status='confirmed');await page.locator('#gdfRefresh').click();await expect(page.locator('#gdfDetail')).toContainText('Expediente cerrado · entregado, facturado y pagado');
 await page.evaluate(()=>Object.assign(__DB.external_invoices[0],{external_status:null,external_number:null}));await page.locator('#gdfRefresh').click();await expect(page.locator('.gdfStep').nth(8)).toHaveClass(/done/);await expect(page.locator('#gdfDetail')).toContainText('Expediente cerrado · entregado, facturado y pagado');
 await page.evaluate(()=>GamaPayments.open=a=>window.__target=a);await page.locator('[data-action="payment"]').click();expect(await page.evaluate(()=>window.__target)).toEqual({invoiceId:'i1'});
});
