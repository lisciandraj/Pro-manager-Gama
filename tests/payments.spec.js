...
test('client terms are loaded and saved centrally, including zero days',async({page})=>{
 await boot(page);await page.evaluate(()=>GamaPayments.open({invoiceId:'i1'}));await page.locator('#gpCustomer').click();await expect(page.locator('#cPaymentTerms')).toHaveValue('30');await page.locator('#cPaymentTerms').fill('0');await page.locator('button[onclick="saveClient()"]').click();await expect.poll(()=>page.evaluate(()=>__DB.customers[0].payment_terms_days)).toBe(0);await page.evaluate(()=>editClient('123456'));await expect(page.locator('#cPaymentTerms')).toHaveValue('0');
});
