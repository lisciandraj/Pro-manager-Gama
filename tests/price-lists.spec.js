...
// La otra mitad del trato: lo que se le COMPRA a un proveedor según su
// contrato. Se cargan de golpe desde Importación Excel y se corrigen aquí
// cuando el contrato cambia, que es para lo que existe esta pestaña.
test.describe('Tarifas especiales — precios de contrato de proveedor', () => {
  const PROVEEDORES = [
    { id: 'sup1', name: 'TecnoSuministros Ecuador', tax_id: '0991234567001', active: true },
    { id: 'sup2', name: 'Logística y Suministros Loja', tax_id: '0992', active: true },
    { id: 'sup3', name: 'Proveedor archivado', tax_id: '0993', active: false },
  ];
  const CONTRATO = { id: 't1', supplier_id: 'sup1', product_id: 'p1', unit_cost: 4.25, contract_ref: 'CTR-2026-01' };

  async function abrirProveedores(page, db = {}) {
    await boot(page, Object.assign({
      products: PRODUCTS, customers: CUSTOMERS, suppliers: PROVEEDORES,
      supplier_contract_prices: [CONTRATO],
    }, db));
    await page.click('#mainmenu .gamaF2Card:has-text("Tarifas")');
    await page.waitForTimeout(700);
    await page.click('.plTabs button:has-text("Proveedores")');
    await page.waitForTimeout(500);
  }

  test('la pestaña lista los proveedores activos y enseña lo pactado con cada uno', async ({ page }) => {
    await abrirProveedores(page);

    await expect(page.locator('.plList')).toContainText('TecnoSuministros Ecuador');
    await expect(page.locator('.plList')).toContainText('Logística y Suministros Loja');
    await expect(page.locator('.plList'), 'un proveedor archivado no compra nada').not.toContainText('Proveedor archivado');

    await page.click('[data-pick="sup1"]');
    await page.waitForTimeout(400);
    // Se compara contra el precio de compra de la ficha (6), no contra el de venta.
    await expect(page.locator('.plTable tbody tr')).toContainText('Cemento 50kg');
    await expect(page.locator('.plTable tbody tr')).toContainText('$6,00');
    await expect(page.locator('.plTable input[data-price="p1"]')).toHaveValue('4.25');
    // Y de qué contrato sale, que es lo que hay que mirar cuando cambie.
    await expect(page.locator('.plTable tbody tr')).toContainText('CTR-2026-01');
  });

  test('el precio se corrige cuando el contrato evoluciona', async ({ page }) => {
    await abrirProveedores(page);
    await page.click('[data-pick="sup1"]');
    await page.waitForTimeout(400);

    await page.fill('.plTable input[data-price="p1"]', '3.90');
    await page.locator('.plTable input[data-price="p1"]').blur();

    await expect.poll(() => page.evaluate(() =>
      window.__DB.supplier_contract_prices.map(x => x.unit_cost))).toEqual([3.9]);
    // Corrige la fila, no añade una segunda para el mismo par.
    expect(await page.evaluate(() => window.__DB.supplier_contract_prices.length)).toBe(1);
  });

  test('añadir y retirar un precio de contrato', async ({ page }) => {
    await abrirProveedores(page);
    await page.click('[data-pick="sup2"]');
    await page.waitForTimeout(400);
    await expect(page.locator('#price-lists')).toContainText('Ningún precio de contrato todavía');

    await page.selectOption('#plProduct', 'p2');
    await page.fill('#plPrice', '15');
    await page.click('#plAdd');
    await expect.poll(() => page.evaluate(() =>
      (window.__DB.supplier_contract_prices.find(x => x.supplier_id === 'sup2') || {}).unit_cost)).toBe(15);

    await page.click('.plTable [data-drop="p2"]');
    await expect.poll(() => page.evaluate(() =>
      window.__DB.supplier_contract_prices.filter(x => x.supplier_id === 'sup2').length)).toBe(0);
    await expect(page.locator('#price-lists')).toContainText('vuelve al precio de compra de la ficha');
  });

  // Las dos pestañas escriben en tablas distintas: tocar una no puede mover la
  // otra. Es el error que haría facturar a un cliente el precio del proveedor.
  test('las tarifas de venta y las de compra no se pisan', async ({ page }) => {
    await abrirProveedores(page, { customer_special_prices: [SPECIAL_C1] });
    await page.click('[data-pick="sup1"]');
    await page.waitForTimeout(400);
    await page.fill('.plTable input[data-price="p1"]', '2.50');
    await page.locator('.plTable input[data-price="p1"]').blur();
    await page.waitForTimeout(500);

    expect(await page.evaluate(() => window.__DB.customer_special_prices.map(x => x.unit_price)))
      .toEqual([8]);
    expect(await page.evaluate(() => window.__DB.supplier_contract_prices.map(x => x.unit_cost)))
      .toEqual([2.5]);
  });
});
