...
async function boot(page,role='admin'){
 ...
 await page.evaluate(async()=>{await window.GamaCloudReady;const original=GamaCloud.db;window.__salesCalls=[];GamaCloud.db=async()=>{const c=await original();return{...c,rpc:async(name,args)=>{if(name!=='gama_sales_action')return c.rpc(name,args);window.__salesCalls.push(args);return window.__salesResponse||{data:{id:window.__DB.sales_orders[0].id},error:null}}}}});
...
test('menu, order quantities and shipment uses the reserved location in one call',async({page})=>{
 ...
 await page.locator('#gsShip').click();await page.locator('[data-dispatch]').fill('60');
 await page.locator('#gsSave').click();await expect(page.locator('dialog')).toHaveCount(0);
 const calls=await page.evaluate(()=>window.__salesCalls);expect(calls).toHaveLength(1);expect(calls[0].p_action).toBe('ship');expect(calls[0].p_data.lines).toEqual([{line_id:ID.line,location_id:ID.location,quantity:60}]);expect(calls[0].p_data.request_key).toMatch(/^[a-f0-9-]{36}$/);
});
...
test('failed mutation keeps entered data and reuses the retry key',async({page})=>{
 await boot(page);await page.locator('[data-gs-order]').first().click();await page.locator('#gsShip').click();await page.locator('[data-dispatch]').fill('60');
 await page.evaluate(()=>window.__salesResponse={error:{message:'INSUFFICIENT_RESERVED'}});await page.locator('#gsSave').click();await expect(page.locator('#gsFormError')).toContainText('reserva ha cambiado');await expect(page.locator('[data-dispatch]')).toHaveValue('60');
