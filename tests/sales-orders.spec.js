...
async function boot(page,role='admin'){
 ...
 await page.evaluate(async()=>{await window.GamaCloudReady;const original=GamaCloud.db;window.__salesCalls=[];window.__commercialCalls=[];GamaCloud.db=async()=>{const c=await original();return{...c,rpc:async(name,args)=>{if(name==='gama_sales_action'){window.__salesCalls.push(args);return window.__salesResponse||{data:{id:window.__DB.sales_orders[0].id},error:null}}if(name==='gama_commercial_action'){window.__commercialCalls.push(args);return window.__commercialResponse||{data:{id:'payment-or-link'},error:null}}return c.rpc(name,args)}}}});
