...
begin
 ...
 insert into public.invoices(customer_id,user_id,quote_state,quote_revision,subtotal,tax,quote_details) values(c,a,null,1,31,4.5,'{}') returning id into q;
 ...
 if exists(select 1 from public.external_invoices where order_id=o) then raise exception 'FAIL_UNACCEPTED_INVOICE';end if;
 if not exists(select 1 from public.tms_deliveries where id=td and status='Entregada' and delivered_at is not null) then raise exception 'FAIL_DELIVERY_ROLLBACK';end if;
 if not exists(select 1 from public.sales_events where order_id=o and action='invoice_auto_failed' and detail->>'error'='QUOTE_ORDER_MISMATCH') then raise exception 'FAIL_BILLING_NOT_RECORDED';end if;
end $$;
