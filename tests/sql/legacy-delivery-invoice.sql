...
do $$
declare a uuid;c uuid;p uuid;q uuid;o uuid;l uuid;td uuid;sd uuid;loc uuid;i jsonb;again jsonb;blocked boolean;warehouse_user uuid;
begin
 ...
 insert into public.invoices(customer_id,user_id,quote_state,quote_revision,subtotal,tax,quote_details) values(c,a,null,1,30,4.5,'{}') returning id into q;
 ...
 blocked:=false;begin perform private.gama_delivery_invoice_create('create',jsonb_build_object('quote_id',q));exception when others then if sqlerrm='DELIVERY_VALIDATION_REQUIRED' then blocked:=true;else raise;end if;end;
 ...
 update public.sales_deliveries set departed_at=now(),departed_by=a where id=sd;
 select id into warehouse_user from public.profiles where role='almacenero' and active limit 1;
 if warehouse_user is null then raise exception 'WAREHOUSE_FIXTURE_REQUIRED';end if;
 perform set_config('request.jwt.claim.sub',warehouse_user::text,true);
 execute 'set local role authenticated';
 update public.tms_deliveries set delivered_at=now() where id=td;
 execute 'reset role';
 select to_jsonb(f) into i from public.external_invoices f where f.order_id=o;
 if i is null then raise exception 'FAIL_AUTO_INVOICE';end if;
 update public.tms_deliveries set delivered_at=now() where id=td;
 if (select count(*) from public.external_invoices where order_id=o)<>1 then raise exception 'FAIL_AUTO_DUPLICATE';end if;
 if has_function_privilege('authenticated','private.gama_delivery_invoice_create(text,jsonb)','execute') then raise exception 'FAIL_PRIVATE_API';end if;
 perform set_config('request.jwt.claim.sub',a::text,true);
 execute 'set local role authenticated';
 select to_jsonb(f) into i from public.external_invoices f where f.order_id=o;
 again:=i;
 ...
 if again->0->>'invoice_id'<>i->>'id' then raise exception 'FAIL_ELIGIBILITY';end if;
 execute 'reset role';
 if (select quote_state from public.invoices where id=q) is not null then raise exception 'FAIL_ACCEPTANCE_CHANGED';end if;
